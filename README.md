...
```
(npm run sync; cd site; vendor/bin/phpunit tests/unit/Espo/Modules/Inventario)
```
...
```
(npm run sync; cd site; vendor/bin/phpunit tests/integration/Espo/Modules/Inventario)
```
...
If your extension contains additional PHP packages, you also need to add `site/custom/Espo/Modules/Inventario/vendor` to the *scanDirectories* section in *phpstan.neon* config.
...
* `site/custom/`
* `site/client/custom/`
* `site/tests/unit/Espo/Modules/Inventario`
* `site/tests/integration/Espo/Modules/Inventario`
...
2. Set *bundled* and *jsTranspiled* to true in `src/files/custom/Espo/Modules/Inventario/Resources/module.json`.
3. Add `src/files/custom/Espo/Modules/Inventario/Resources/metadata/app/client.json`
    ```json
    {
        "scriptList": [
            "__APPEND__",
            "client/custom/modules/inventario/lib/init.js"
        ]
    }
    ```
...
Add the library module path to `src/files/custom/Espo/Modules/Inventario/Resources/metadata/app/jsLibs.json`

```json
{
    "some-lib": {
        "path": "client/custom/modules/inventario/lib/some-lib.js"
    }
}
```
